// ---------- PRODUCT DATABASE ----------
    const products = [
        { id: 1, name: "HP PROBOOK 460 G11 NOTEBOOK INTEL CORE ULTRA 7-155U, 8GB DDR5, 512GB SSD, INTEGRATED INTEL GRAPHICS, 16″ WUXGA IPS 300NITS NON-TOUCH, HP PREMIUM BACKLIT KYB W/ NUMPAD, FINGERPRINT READER, WIN 11", description:"", brand: "HP", price: 12500, originalPrice: 13500, discount: "26%", image: "ProductImages/hp probook 460.webp" },
    ...
    function saveCart() {
        localStorage.setItem('Bridge-Well Africa Limited_cart', JSON.stringify(cart));
    }
    
    function loadCart() {
        const saved = localStorage.getItem('Bridge-Well Africa Limited_cart');
        if (saved) {
    ...
    function updateCartUI() {
        ...
        if (cart.length === 0) {
            cartContainer.innerHTML = '<div class="empty-cart-msg">Your cart is empty. Add some Laptops! 💻</div>';
            return;
        }
    ...
    if (checkoutForm) {
        checkoutForm.addEventListener('submit', function(e) {
            ...
            const body = `Hello Bridge-Well Africa,\n\nI would like to place an order:\n\n${orderDetails}\n\nTotal: ₵${totalOrder}\n\nMy details:\n- Name: ${name}\n- Email: ${email}\n- Address: ${address}\n- Phone: ${phone}\n\nPlease confirm.`;
            const mailtoLink = `mailto:support@bridgewellafrica.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
